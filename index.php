...
if ($folder_list) $folder_list = php_multisort($folder_list, $sort);
if ($file_list) $file_list = php_multisort($file_list, $sort);
...
<body>
  <div class="container-fluid">
    <h1>
      <a href="//<?= $this_domain ?>"><?= $this_domain ?></a><? foreach ($dir_name as $dir => $name): ?><? if (($name != ' ') && ($name != '') && ($name != '.') && ($name != '/')):?><? $parent = ''; ?><? for ($i = 1; $i <= $dir; $i++): ?><? $parent .= $dir_name[$i] . '/';?><?endfor;?> / <a href=/<?= $parent ?>><?= $name ?></a><?endif;?><?endforeach;?>
    </h1>
  </div>
  <div class="table-responsive">
    <table class="table">
      ...
      <tbody><?if ($folder_list):?><? foreach ($folder_list as $item): ?>
        ...
        <? endforeach; ?><? endif; ?><? if ($file_list): ?><? foreach ($file_list as $item): ?>
        <tr>
          <th>
